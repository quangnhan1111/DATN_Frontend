  ...
  <div class="wrapper">
    ...
    <form class="form" @submit.prevent="handleEdit">
      <div class="inputfield">
        <label>Id</label>
        <input type="text" class="input" v-model="brand.id" disabled>
      </div>
      <div class="inputfield">
        <label>Name</label>
      ...
      <div class="inputfield">
        <label>Status</label>
        <b-form-select v-model="brand.status" :options="Status"></b-form-select>
      </div>
      <div class="inputfield">
...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      console.log(id)
      await axios.get('brand/' + id, token).then((response)=> {
    ...
    checkValid() {
      ...
      }else if(this.brand.name.length <3){
    ...
    async handleEdit() {
      ...
      if(this.message.length<=0) {
        ...
        let id = this.$route.params.id
        console.log(data)
        await axios.put('brands/?id=' + id, data, token).then((response) => {
