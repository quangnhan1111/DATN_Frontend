    ...
    <div class="table_responsive">
      <div class="header-table">
        <div class="header-table__item">
          <strong class="title"> List brands </strong>
          <input
              class="input-search"
              placeholder="Search..."
              v-model="searchValue"
        ...
        <tr v-for="item in searchValue.length < 1 ? listBrand : searchResults" :key="item.id">
          ...
          <td>
            <span class="action_btn">
               <router-link :to="`/edit-brand/${item.id}`"
                            tag="button" class="btn"
               >Edit
              </router-link>
              <button class="btn" @click="deleteBrand(item.id)">Remove</button>
            </span>
...
export default {
  ...
  methods: {
    ...
    async ToggleStatus(id){
      ...
      await axios.get('brand-activation/' + id, token).then((response) => {
        console.log(response.data)
        toast({
          message: 'Toggle status Brand successfully',
          type: 'is-success',
          dismissible: true,
          pauseOnHover: true,
          duration: 3000,
    ...
    async deleteBrand(id) {
      ...
      await axios.delete('brands/?id=' + id,token)
          .then(response => {
            this.flag = id
            console.log(response.data)
            toast({
