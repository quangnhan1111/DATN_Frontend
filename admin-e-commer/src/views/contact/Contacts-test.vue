    ...
    <div class="main-container">
      <div class="room-sidebar">
        ...
        <div v-for="group in listGroup" :key="group.id">
          <div class="room-sidebar-groups" @click="changGroup(group.groups_participated.id)">
      ...
      <div id='room-message'>
        <div class="room-container__msg">
          ...
          <div class="msg-header mat-elevation-z6">
            ...
            <div class="h-users">
              <div class="h-user-group">{{listGroup[0]['groups_participated']['name']}}</div>
              <div class="h-user-member">{{listGroup[0]['groups_participated']['users']}}</div>
            </div>
...
export default {
  ...
  methods: {
    ...
    connect(){
      // let roomId = this.listGroup[0]['groups_participated']['id']
      ...
      chatSocket.onmessage = function(e) {
        ...
        console.log(data)
        this.broadcastMessage(data.message, data.username, data.message_type, data.image_caption)
        this.scrollBottom()
      };
      ...
      document.getElementById('btnClosePreviewImg').onclick = function() {
        this.goRoomMsg();
      }

      document.getElementById('btnSendImg').onclick = function() {
        this.sendImage(chatSocket);
        this.goRoomMsg();
      }
    ...
    broadcastMessage(msg, user, msg_type, img_caption, msg_time){
      ...
      if (user == this.listGroup[0]['groups_participated']['username_of_current_user']) {
        msg1 = `<div class="right-msg-container">
                                <div class="s-message">
                                    <div class="s-msg">${msg}</div>
                                    <div class="s-time">${this.getTime(msg_time)}</div>
                                </div>
        ...
        msg1 = `<div class="left-msg-container">
                                ...
                                <div class="r-message" >
                                    ...
                                    <div class="r-time">${this.getTime(msg_time)}</div>
                                </div>
                            </div>`
      }

      if (msg_time) {
        this.showDatesWeekDays(msg_time)
      } else {
        this.showDatesWeekDays(new Date())
      }
      ...
      this.setSidebarMessage(msg_type, user, msg);
    },
    goRoomMsg() {
    ...
    setSidebarMessage(msg_type, user, msg) {
      // side bar message
      if (msg_type == 'image') {
        document.getElementById('rsb-g'+this.listGroup[0]['groups_participated']['id']).innerHTML = 'Photo';
      } else {
        document.getElementById('rsb-g'+this.listGroup[0]['groups_participated']['id']).innerHTML = `${user}: ${msg}`;
      }
      document.getElementById('rsb-t'+this.listGroup[0]['groups_participated']['id']).innerHTML = this.getTime();
    },
    ...
    showDatesWeekDays(date_created) {
      // add the newly created element and its content into the DOM
      let dt = new Date(date_created)
      let date_weekday = '';
      if (!this.tempDaysWeekdays.includes(dt.toLocaleDateString())) {
        let newDiv = document.createElement("div");
        let currentDiv = document.getElementById("new-message-chat");
        let parentDiv = currentDiv.parentNode;
        ...
        this.tempDaysWeekdays.push(dt.toLocaleDateString())
      }

    },
    getDateBefore(days=7) {
    ...
    loadMessage() {
      axios
          .get('history/'+this.listGroup[0]['groups_participated']['id'])
          .then(response => {
