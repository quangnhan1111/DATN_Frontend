    ...
    <div class="table_responsive">
      <div class="header-table">
        <div class="header-table__item">
          <strong class="title"> List Employees </strong>
          <input
              class="input-search"
              placeholder="Search..."
              v-model="searchValue"
        ...
        <div class="header-table__item">
          <template v-if="auth == 1">
            <router-link to="/add-employee"  tag="button" class="btn btn-dark">New Employees</router-link>
          </template>
        </div>
      </div>
      <table>
...
<!--          <td>{{item.full_name}}</td>-->
          ...
          <td>
            <span class="action_btn">
               <router-link :to="`/edit-employee/${item.id}`"
                            tag="button" class="btn"
               >Edit
              </router-link>
              <button class="btn" @click="deleteEmployee(item.id)">Remove</button>
            </span>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      console.log(token)
      axios
          .get('admin/staffs?page='+this.paginate.current_page, token)
          .then(response => {
    ...
    async ToggleStatus(id){
      ...
      await axios.get('staff-activate/' + id, token).then((response) => {
    ...
    async deleteEmployee(id) {
      ...
      await axios.delete('admin/staffs?id=' + id,token)
          .then(response => {
            this.flag = id
            console.log(response.data)
            toast({
