    ...
    <div class="table_responsive">
      <div class="header-table">
        <div class="header-table__item">
          <strong class="title"> List Chat Room </strong>
          <input
              class="input-search"
              placeholder="Search..."
              v-model="searchValue"
        ...
        <div class="header-table__item">
          <template v-if="auth == 1">
            <router-link to="/add-chat-room"  tag="button" class="btn btn-dark">New Chat Room</router-link>
          </template>
        </div>
      </div>
      <table>
        ...
        <tbody>
        <tr v-for="item in searchValue.length < 1 ? listChatRoom : searchResults" :key="item.id">
          <td>{{item.id}}</td>
          <td>{{item.name}}</td>
          ...
          <td>
            <span class="action_btn">
               <router-link :to="`/edit-chat-room/${item.id}`"
                            tag="button" class="btn"
               >Edit
              </router-link>
              <button class="btn" @click="deleteChatRoom(item.id)">Remove</button>
            </span>
...
<script>
// import CTableWrapper from '../../components/chatroom/chat_room'
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      // this.checklogin();
      axios
          .get('/chat-rooms?page='+this.paginate.current_page, token)
          .then(response => {
            console.log(response.data.data)
            this.all = response.data
            this.listChatRoom = this.all.data
    ...
    async ToggleStatus(id){
      ...
      await axios.get('chat-room-activate/' + id, token).then((response) => {
        console.log(response.data)
        toast({
          message: 'Toggle status Chat Room successfully',
          type: 'is-success',
          dismissible: true,
          pauseOnHover: true,
          duration: 3000,
    ...
    searchHandler(){
      if (this.searchValue !== "") {
        ...
        // });
        this.searchResults = Object.values(this.listChatRoom).filter((chatroom) => {
          return Object.values(chatroom)
              .join(" ")
              .toLowerCase()
              .includes(this.searchValue.toLowerCase());
        });
      } else {
        this.searchResults = this.listChatRoom;
      }
    },
    async deleteChatRoom(id) {
      ...
      await axios.delete('chat-rooms?id=' + id,token)
          .then(response => {
            console.log(response.data)
            this.flag = id
            toast({
