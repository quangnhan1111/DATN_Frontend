    ...
    <div class="table_responsive">
      <div class="header-table">
        <div class="header-table__item">
          <strong class="title"> List Coupons </strong>
          <input
              class="input-search"
              placeholder="Search..."
              v-model="searchValue"
        ...
        <div class="header-table__item">
          <template >
            <router-link to="/add-coupon"  tag="button" class="btn btn-dark">New Coupons</router-link>
          </template>
        </div>
      </div>
      <table>
        ...
        <tbody>
        <tr v-for="item in searchValue.length < 1 ? listCoupon : searchResults" :key="item.id">
          <td>{{item.id}}</td>
          <td>{{item.name}}</td>
          ...
          <td>
            <span class="action_btn">
               <router-link :to="`/edit-coupon/${item.id}`"
                            tag="button" class="btn"
               >Edit
              </router-link>
              <button class="btn" @click="deleteCoupon(item.id)">Remove</button>
            </span>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      // this.checklogin();
      axios
          .get('/coupons?page='+this.paginate.current_page, token)
          .then(response => {
            console.log(response.data.data)
            this.all = response.data
            this.listCoupon = this.all.data
    ...
    async ToggleStatus(id){
      ...
      await axios.get('coupon-activate/' + id, token).then((response) => {
        console.log(response.data)
        toast({
          message: 'Toggle status Coupon successfully',
          type: 'is-success',
          dismissible: true,
          pauseOnHover: true,
          duration: 3000,
    ...
    searchHandler(){
      if (this.searchValue !== "") {
        ...
        // });
        this.searchResults = Object.values(this.listCoupon).filter((coupon) => {
        ...
        this.searchResults = this.listCoupon;
      }
    },
    async deleteCoupon(id) {
      ...
      await axios.delete('coupons?id=' + id,token)
          .then(response => {
            this.flag = id
            console.log(response.data)
            toast({
